...
if [[ "$sdk" -lt 23 ]]; then
  ...
  clear
  rm -rf $output_path
  echo "Sorry. System not supported!"
  echo " "
  return 1
fi
umount /system
...
for f in $partitions; do
  ...
  skip=0
  if [ $max_blocks -gt 0 -a $size -gt $max_blocks ]; then
    skip=1
...
echo "  </digests>" >>$output_path/md5sum.xml
echo "</root>" >>$output_path/md5sum.xml
find $output_path/md5sum.xml | grep -v 'bk' $output_path/md5sum.xml >>$output_path/md5sum1.xml
rm -rf $output_path/md5sum.xml
mv $output_path/md5sum1.xml $output_path/md5sum.xml
