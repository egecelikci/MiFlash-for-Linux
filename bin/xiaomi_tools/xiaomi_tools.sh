...
"
	...
	c | C)
		adb kill-server
		adb start-server
		if [[ $(adb devices | grep "device" -c) -gt "1" ]]; then adb reboot; else adb reboot; fi
		clear
		;;
	g | G)
		adb kill-server
		adb start-server
		if [[ $(adb devices | grep "device" -c) -gt "1" ]]; then adb reboot recovery; else adb reboot recovery; fi
		clear
		;;
	y | Y)
		adb kill-server
		adb start-server
		if [[ $(adb devices | grep "device" -c) -gt "1" ]]; then adb reboot bootloader; else adb reboot bootloader; fi
		clear
		;;
	k | K)
		adb kill-server
		adb start-server
		if [[ $(adb devices | grep "device" -c) -gt "1" ]]; then adb reboot edl; else adb reboot edl; fi
		clear
		;;
	*) clear ;;
	esac
...
seltxiaomitgz() {
	if [[ ! $(fastboot devices) ]]; then
		clear
		poster
		echo -e $r"$t_Restart_phone_fastboot_mode!"$c
		pause
		clear
		poster
		navbar_reboot
	...
	if [[ -z $fileList1 ]]; then
		clear
		echo
		echo -e "$r TGZ/ZIP: $t_Check_files: XIAOMI-FILE$c"
		pause
		clear
		xiaomi_menu
		break
		...
		select fileName in $fileList; do
			if [[ -n "$fileName" ]]; then
				fileName=${fileName%.*}
				if [[ $1 == "s1" ]]; then clear; fi
				if [[ $fileName == $fileExit ]]; then
					clear
					xiaomi_menu
				fi
				if [[ ! -f "$bin/start.cfg" ]]; then
					if [[ $fileName == $fileQuit ]]; then
						clear
						quit
					fi
				else
...
rom_tgz() {
	...
	if [[ $(fastboot devices) ]]; then fastboot devices; fi
	echo
	echo "$t_Your_selected_file $fileName.tgz"
	echo " "
...
search_recovery() {
	...
	if [[ $(fastboot devices) ]]; then fastboot devices; fi
	echo
	echo -e "$t_Your_selected_file $y $fileName.zip$c"
	echo " "
	...
	if [[ -z $fileList1 ]]; then
		clear
		echo
		echo -e "$r recovery.img: $t_Check_files /XIAOMI-FILE/recovery.img$c"
		pause
		clear
		seltxiaomitgz
		break
		...
		select fileNameTWRP in $fileList; do
			if [[ -n "$fileNameTWRP" ]]; then
				...
				if [[ $1 == "s1" ]]; then clear; fi
				if [[ $fileNameTWRP == $fileExit ]]; then
					clear
					seltxiaomitgz
				fi
				if [[ ! -f "$bin/start.cfg" ]]; then
					if [[ $fileNameTWRP == $fileQuit ]]; then
						clear
						quit
					fi
				else
				...
				echo
				echo "$t_Your_selected_file $fileNameTWRP.img"
				if [ ! -d "$sourcetgz/firmware_temp" ]; then
					mkdir $sourcetgz/firmware_temp
					mkdir $sourcetgz/firmware_temp/tmp
					mkdir $sourcetgz/firmware_temp/tmp/images
				fi
...
rom_zip() {
	...
	if [[ $(fastboot devices) ]]; then fastboot devices; fi
	echo
	echo "$t_Your_selected_file $fileName.zip"
	echo " "
	if [ ! -d "$sourcetgz/firmware_temp" ]; then
		mkdir $sourcetgz/firmware_temp
		mkdir $sourcetgz/firmware_temp/tmp
	...
	grep "\-update\/" $sourcetgz/firmware_temp/tmp/flash_all11 | cut -d"/" -f2 >$sourcetgz/firmware_temp/tmp/flash_all111
	sed -i 's/"//g' $sourcetgz/firmware_temp/tmp/flash_all111
	sed -i "s/^/ICE0\/images\//g" $sourcetgz/firmware_temp/tmp/flash_all111

	grep "name/" $sourcetgz/firmware_temp/tmp/flash_all00 | cut -d"," -f2 >$sourcetgz/firmware_temp/tmp/flash_all22
	cut -d"-" -f2 $sourcetgz/firmware_temp/tmp/flash_all22 >$sourcetgz/firmware_temp/tmp/flash_all220
	grep "name/" $sourcetgz/firmware_temp/tmp/flash_all220 | cut -d"/" -f2 >$sourcetgz/firmware_temp/tmp/flash_all222
	sed -i 's/");//g' $sourcetgz/firmware_temp/tmp/flash_all222
	sed -i "s/^/fastboot ICE1 flash /g" $sourcetgz/firmware_temp/tmp/flash_all222

	paste $sourcetgz/firmware_temp/tmp/flash_all222 $sourcetgz/firmware_temp/tmp/flash_all111 >$sourcetgz/firmware_temp/tmp/flash_rom_zip.sh
	rm -rf $sourcetgz/firmware_temp/tmp/*flash_all2*
	rm -rf $sourcetgz/firmware_temp/tmp/*flash_all1*
	rm -rf $sourcetgz/firmware_temp/tmp/*flash_all0*
	rm -rf $sourcetgz/firmware_temp/tmp/updater-script
	...
	rm -r -d $sourcetgz/firmware_temp/firmware-update
	rm -r -d $sourcetgz/firmware_temp/META-INF
	echo " "
	clear
	if [[ ! -f $sourcetgz/firmware_temp/tmp/images/system.img ]]; then
		clear
		poster
		echo -e "$r ZIP $t_Not_supported$c"
		pause
...
seltxiaomish() {
	...
	if [[ -z $fileList1 ]]; then
		clear
		echo
		echo -e "$r SH: $t_Check_files /XIAOMI-FILE/$fileName/*.sh$c"
		pause
		clear
		xiaomi_menu
		break
		...
		select fileName in $fileList; do
			if [[ -n "$fileName" ]]; then
				...
				if [[ $fileName == $fileExit ]]; then
					clear
					rm -d -r "$PWD/XIAOMI-FILE/firmware_temp"
					seltxiaomitgz
				fi
				if [[ ! -f "$bin/start.cfg" ]]; then
					if [[ $fileName == $fileQuit ]]; then
				...
				if [[ $1 == "s1" ]]; then clear; fi
				echo
				echo "$t_Your_selected_file  $fileName.sh"
				$fileName.sh
				echo "$t_Done"
...
backup_partitions() {
	if [[ $(adb devices | grep "device" -c) -gt "1" ]]; then
		clear
		poster
		echo -e $r"$t_Restart_phone_twrp_mode!"$c
		pause
		clear
		poster
		navbar_reboot
	...
	if [[ $(adb devices | grep "recovery" -c) = "0" ]]; then
		clear
		poster
		echo -e $r"$t_Pulling_error_device!"$c
		pause
		clear
		xiaomi_menu
	fi
	...
	if [[ -z $fileList1 ]]; then
		...
		select fileName in $fileList; do
			if [[ -n "$fileName" ]]; then
				...
				echo
				echo "$t_Your_selected_file $fileName.img"
...
flash_boot() {
	...
	if [[ -z $fileList1 ]]; then
		clear
		echo
		echo -e "$r boot.img: $t_Check_files /XIAOMI-FILE/boot.img$c"
		pause
		clear
		xiaomi_menu
		break
...
xiaomi_menu() {
	...
	if [[ $(adb devices) ]]; then adb devices; fi
	if [[ $(fastboot devices) ]]; then fastboot devices; fi
	if [ ! -d "$PWD/XIAOMI-FILE" ]; then mkdir XIAOMI-FILE; fi
	if [ -d "$PWD/XIAOMI-FILE/firmware_temp" ]; then rm -d -r "$PWD/XIAOMI-FILE/firmware_temp"; fi
	echo -e "
"=========================================================================================="
...
"=========================================================================================="
  99 - $y$t_Exit$c | H -$y Help$c | L - $y$t_Language$c | Z -$y $t_Restart_menu$c
"=========================================================================================="
"
	printf %s "$t_Enter_option: "
	read rxiaomi
	...
	h | H) help ;;
	l | L) language ;;
	99 | 99) if [[ ! -f "$bin/start.cfg" ]]; then quit; else
		clear
		exit
	fi ;;
	z | Z) if [[ $(fastboot devices) ]]; then
		clear
		poster
		navbar_reboot_fastboot
		xiaomi_menu
